<?php

/**
 * ECSHOP 文章管理語言項
 * ============================================================================
 * 版權所有 (C) 2005-2006 北京億商互動科技發展有限公司，並保留所有權利。
 * 網站地址: http://www.ecshop.com
 * ----------------------------------------------------------------------------
 * 這是一個免費開源的軟件；這意味著您可以在不用於商業目的的前提下對程序代碼
 * 進行修改、使用和再發佈。
 * ============================================================================
 * $Author: refly $
 * $Date: 2007-11-26 21:10:53 +0800 (星期一, 26 十一月 2007) $
 * $Id: article.php 13757 2007-11-26 13:10:53Z refly $
*/

$_LANG['title']         ='文章標題';
$_LANG['cat']           ='文章分類';
$_LANG['article_type']  ='文章重要性';
$_LANG['author']        ='文章作者';
$_LANG['email']         ='作者email';
$_LANG['keywords']      ='關鍵字';
$_LANG['content']       ='文章內容';
$_LANG['is_open']       ='是否顯示';
$_LANG['article_id']    ='編號';
$_LANG['add_time']      ='發表日期';
$_LANG['upload_file']   ='上傳檔案';
$_LANG['file_url']      ='或輸入檔案地址';
$_LANG['invalid_file']  = '上傳檔案格式不正確';
$_LANG['article_name_exist']  = '有相同的文章名稱存在!';
$_LANG['select_plz']    = '請選擇...';
$_LANG['external_links'] = '外部連結';

$_LANG['top']            ='置頂';
$_LANG['common']         ='普通';
$_LANG['isopen']         ='顯示';
$_LANG['isclose']        ='不顯示';
$_LANG['no_article']       = '您現在還沒有任何文章';
$_LANG['no_select_article']       = '您沒有選擇任何文章';

$_LANG['display']       = '顯示文章內容';
$_LANG['download']      = '下載檔案';
$_LANG['both']          = '既顯示文章內容又下載檔案';

$_LANG['button_remove']  ='批次刪除';

$_LANG['article_edit']          = '編輯文章內容';
$_LANG['tab_general']           = '一般設定';
$_LANG['tab_content']           = '文章內容';
$_LANG['tab_goods']             = '關聯商品';

$_LANG['link_goods']            = '跟該文章相關的商品';
$_LANG['keyword']               = '關鍵字';

/* 提示信息 */
$_LANG['title_exist']           ='文章 %s 已經存在';
$_LANG['back_list']             ='返回文章列表';
$_LANG['continue_add']          ='繼續發表新文章';
$_LANG['articleadd_succeed']    ='文章已經發表成功';
$_LANG['articleedit_succeed']   ='文章 %s 成功編輯';
$_LANG['articleedit_fail']      ='文章編輯失敗';
$_LANG['no_title']              ='沒有輸入文章標題';
$_LANG['drop_confirm']          = '您確認要刪除這篇文章嗎？';
$_LANG['batch_remove_succeed']  = '您已經成功刪除 %d 篇文章';


/*JS 語言項*/
$_LANG['js_languages']['no_title']     = '沒有文章標題';
$_LANG['js_languages']['no_cat']       = '沒有選擇文章分類';
$_LANG['js_languages']['not_allow_add'] = '系統保留分類，您無法在該分類加入文章';

$_LANG['all_cat'] = '全部分類';

?>