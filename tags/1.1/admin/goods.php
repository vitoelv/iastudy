...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        if (!empty($goods['course_start_date']))
        {
            $goods['course_start_date']    = trim_right($goods['course_start_date']);
        }  
        if (!empty($goods['course_start_time']))
        {
            $goods['course_start_time']    = trim_right($goods['course_start_time']);
        }
        if (!empty($goods['course_recruit_goal']))
        {
            $goods['course_recruit_goal']    = trim_right($goods['course_recruit_goal']);
        }                
        if (!empty($goods['course_material']))
        {
            $goods['course_material']    = trim_right($goods['course_material']);
        }        		
        if (!empty($goods['course_goal']))
        {
            $goods['course_goal']    = trim_right($goods['course_goal']);
        }
        if (!empty($goods['course_teacher_desc']))
        {
            $goods['course_teacher_desc']    = trim_right($goods['course_teacher_desc']);
        }
        if (!empty($goods['course_address']))
        {
            $goods['course_address']    = trim_right($goods['course_address']);
        }        		
        if (!empty($goods['course_address_map']))
        {
            $goods['course_address_map']    = trim_right($goods['course_address_map']);
        }
        if (!empty($goods['course_telephone']))
        {
            $goods['course_telephone']    = trim_right($goods['course_telephone']);
        }
        if (!empty($goods['course_desc']))
        {
            $goods['course_desc']    = trim_right($goods['course_desc']);
        }              
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	$course_start_date = empty($_POST['course_start_date']) ? '' : $_POST['course_start_date'];
	$course_start_time = empty($_POST['course_start_time']) ? '' : $_POST['course_start_time'];	
	$course_recruit_goal = empty($_POST['course_recruit_goal']) ? '' : $_POST['course_recruit_goal'];	
	$course_material = empty($_POST['course_material']) ? '' : $_POST['course_material'];
	$course_goal = empty($_POST['course_goal']) ? '' : $_POST['course_goal'];	
	$course_teacher_desc = empty($_POST['course_teacher_desc']) ? '' : $_POST['course_teacher_desc'];	
	$course_address = empty($_POST['course_address']) ? '' : $_POST['course_address'];
	$course_address_map = empty($_POST['course_address_map']) ? '' : $_POST['course_address_map'];	
	$course_telephone = empty($_POST['course_telephone']) ? '' : $_POST['course_telephone'];	
	$course_desc = empty($_POST['course_desc']) ? '' : $_POST['course_desc'];
    ...
    if ($is_insert)
    {
        ...
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, goods_desc, add_time, last_update, goods_type, course_start_date, ".
            		"course_start_time,course_recruit_goal,course_material,course_goal,course_teacher_desc,course_address,course_address_map,course_telephone,course_desc)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type','$course_start_date',".
                    " '$course_start_time','$course_recruit_goal','$course_material','$course_goal','$course_teacher_desc','$course_address','$course_address_map','$course_telephone','$course_desc')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, goods_desc, add_time, last_update, goods_type, extension_code, course_start_date, " .
            		"course_start_time,course_recruit_goal,course_material,course_goal,course_teacher_desc,course_address,course_address_map,course_telephone,course_desc)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code','$course_start_date'," .
            		" '$course_start_time','$course_recruit_goal','$course_material','$course_goal','$course_teacher_desc','$course_address','$course_address_map','$course_telephone','$course_desc')";
        }
    }
    else
    {
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
				...
				"course_recruit_goal = '$course_recruit_goal', " .        	        	
				"course_material = '$course_material', " .
				"course_goal = '$course_goal', " .
				"course_teacher_desc = '$course_teacher_desc', " .        	        	
				"course_address = '$course_address', " .
				"course_address_map = '$course_address_map', " .
				"course_telephone = '$course_telephone', " .        	        	        	        	
